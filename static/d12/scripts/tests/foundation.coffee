...
test('Foundation.View', ->
	view = new Foundation.View()

	# Test adding and removing childViews
	childView1 = new Foundation.View()
	childView2 = new Foundation.View()
	view.addChildView(childView1, anchor: "childView1")
	view.addChildView(childView2, anchor: "childView2")
	...
	equals(view.getChildView(childView1.cid), undefined, "ChildView1 removed ok")
	equals(view.getChildView(childView2.cid), childView2, "ChildView2 still there")
	view.removeChildView(childView2)
	equals(view.getChildView(childView1.cid), undefined, "ChildView1 still not there")
	equals(view.getChildView(childView2.cid), undefined, "ChildView2 removed ok")
	view.addChildView(childView1, anchor: "childView1")
	equals(view.getChildView(childView1.cid), childView1, "ChildView1 added")
	equals(view.getChildView(childView2.cid), undefined, "ChildView2 still not there")
	equals(view.getChildView("sdfsdfsdf"), null, "Trying to get unattached childView")
)
...
test('TemplateView', ->
	...
	parentTView.addChildView(childTView1, anchor: "childView1")
	...
	parentTView.addChildView(childTView2, anchor: "childView2")
	...
	parentTView.removeChildView(childTView1)
	equals(parentTView.render().$el.html(),'parent<div data-anchor="childView1">ShouldBeRemoved</div><div data-anchor="childView2"></div>', "All anchored ChildViews removed, renders OK")
)
...
test('ModelView', ->
	...
	equals(
		$(mView.el).html()
		"<h1>Awesome post</h1><div><ul><li>Postlist 1</li><li>Postlist 2</li></ul></div><ul><li>awesome</li><li>post</li></ul>Published"
		"Model template output ok"
	)
	equals(
		$(mView.el).data("modelId")
		post.id
		"Model id saved in element data-attribute"
	)

	# Rerender on model change
	post.set(title:"New Title", id: 2)
	equals(
		$(mView.el).html()
		"<h1>New Title</h1><div><ul><li>Postlist 1</li><li>Postlist 2</li></ul></div><ul><li>awesome</li><li>post</li></ul>Published"
		"Model template rerendered on change"
	)
	equals(
		$(mView.el).data("modelId")
		post.id
		"Model id re-saved in element data-attribute"
	)

	# Rerender on new model
	...
	equals(
		$(mView.el).html()
		"<h1>Another Title</h1><div><ul><li>Postlist 1</li><li>Postlist 2</li></ul></div><ul><li>awesome</li><li>post</li></ul>Published"
		"New Model template rerendered on change"
	)
	equals(
		$(mView.el).data("modelId")
		anotherPost.id
		"New Model id saved in element data-attribute"
	)
