...
(function(undefined) {
	...
	Backbone.RelationalModel = Backbone.Model.extend({
		...
		parseAttributes : function(attributes){
			// Added to parse dateTimeAttributes
			if (attributes && this.dateTimeAttributes){
				_(this.dateTimeAttributes).each(function(dateAttr){
					if (attributes[dateAttr] && _(attributes[dateAttr]).isString()){
						var momentDate =  moment(parseFloat(attributes[dateAttr]*1000));
						if (momentDate) momentDate.native();
						attributes[dateAttr] = momentDate ? momentDate.native() : null;
					}
				});
			}
			return attributes;
		...
		toJSON: function(options) {
			...
			_.each( this._relations, function( rel ) {
				var value = json[ rel.key ];
					if ( ( options.all || rel.options.includeInJSON ) === true && value && _.isFunction( value.toJSON ) ) {
						this.acquire();
						json[ rel.key ] = value.toJSON(options);
						this.release();
					}
